#!/usr/bin/env python3
"""GPT Image 2.0（gpt-image-2）で LP の縦長スライスを 1 枚生成する最小スクリプト。
...
使い方:
  python3 scripts/gen_image.py --prompt "スマホLPのファーストビュー。..." \
      --size 1024x1536 --out assets/slice1.png

サイズ:
  gpt-image-2 は 1024x1536（縦）などに対応。幅・高さは 16 の倍数、アスペクト比 1:3〜3:1。
  縦長スライスは 1024x1536 / 1024x2048 などが目安。
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Generate one vertical LP slice with GPT Image 2.0")
    ap.add_argument("--prompt", required=True, help="画像生成プロンプト")
    ap.add_argument("--size", default="1024x1536", help="例 1024x1536 / 1024x2048")
    ap.add_argument("--out", required=True, help="保存先パス（例 assets/slice1.png）")
